...
with open('ecg1.csv', 'r') as csv_file:
    csv_reader = csv.reader(csv_file)
    for line in csv_reader:
        y.append(np.float64(line[1]))
...
class ScrollingTimestampPlot(QtGui.QWidget):
    ...
    def __init__(self, parent=None):
        ...
        # self.SCROLLING_TIMESTAMP_PLOT_REFRESH_RATE = 1 / (1000) * 1000
        ...
        # Create Plot Widget
        self.scrolling_timestamp_plot_widget = pg.PlotWidget(
            axisItems={'bottom': TimeAxisItem(orientation='bottom')})

        # Enable/disable plot squeeze (Fixed axis movement)
        self.scrolling_timestamp_plot_widget.plotItem.setMouseEnabled(
            x=True, y=True)
        self.scrolling_timestamp_plot_widget.setTitle(
            'Scrolling Timestamp Plot Example')
    ...
    def start(self):
        ...
        self.position_update_timer.start(
            self.get_scrolling_timestamp_plot_refresh_rate())

    def read_position_thread(self):
        """Read in data using a thread"""

        self.current_position_value = 0
        self.position_update_thread = Thread(
            target=self.read_position, args=())
        self.position_update_thread.daemon = True
        self.position_update_thread.start()

    def read_position(self):
        frequency = self.get_scrolling_timestamp_plot_frequency()
        while self.current_position_value < len(y):
            self.draw = y[self.current_position_value]
            self.current_position_value += 1
            print(self.current_position_value)
            time.sleep(frequency)
        self.position_update_thread.daemon = False

    def plot_updater(self):
        self.data_point = float(self.draw)

        self.data.append({'x': self.timestamp.elapsed(), 'y': self.data_point})
        self.scrolling_timestamp_plot.setData(x=[item['x'] for item in self.data], y=[
                                              item['y'] for item in self.data])
...
if __name__ == '__main__':
    ...
    ml.addLayout(
        scrolling_timestamp_plot_widget.get_scrolling_timestamp_plot_layout(), 0, 0)
    mw.show()
